...
def train(args):
    ...
    # Load data
    train_images = sorted(
        glob.glob(os.path.join(args.data_dir, "imagesTr", "*.nii.gz")))
    train_labels = sorted(
        glob.glob(os.path.join(args.data_dir, "labelsTr", "*.nii.gz")))
    data_dicts = [
    ...
    shutil.copy('./inference.py', model_code_dir) ## copy the inference file
    shutil.copy('./requirements.txt', model_code_dir) # copy requirement.txt
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-dir', type=str, default=os.environ.get('SM_CHANNEL_TRAINING') if 'SM_CHANNEL_TRAINING' in os.environ else '/opt/ml/input/')
