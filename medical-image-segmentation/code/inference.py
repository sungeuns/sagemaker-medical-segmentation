...
def input_fn(serialized_input_data, content_type):
    ...
    if content_type == 'application/json':
        ...
        local_dir = "/opt/ml"
        # local_dir = "tmp"  # read-only have an error
        ...
        try:
            ...
            print(f"#### Failed to download file from s3 : {source}")
            print("#### Exception => ", e)
...
def predict_fn(input_data, model):
    ...
    if isinstance(nslice, int):
        ...
        s3_upload = s3.Bucket(bucket).upload_file(local_path, object_name)
        s3_path = os.path.join(f"S3://{bucket}", object_name)
        # os.remove(local_path) ## delete the file after uploading
